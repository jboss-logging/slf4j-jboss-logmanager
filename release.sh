...
if git rev-parse "v${RELEASE_VERSION}" >/dev/null 2>&1; then
    if [ "${FORCE}" == "false" ]; then
        failNoHelp "Tag v${RELEASE_VERSION} already exists. Use -f to force or choose a different version."
    else
        echo -e "${YELLOW}Warning: Tag v${RELEASE_VERSION} already exists but continuing due to -f flag.${CLEAR}"
    fi
fi
...
if command -v gh &>/dev/null; then
    ...
    if ${VERBOSE}; then
        echo "Checking for GitHub CLI default repository..."
    fi
